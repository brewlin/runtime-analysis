...
// 大意是初始化全局root根参数，决定了后面根需要从哪里开始扫描
//go:nowritebarrier
...
func markrootBlock(b0, n0 uintptr, ptrmask0 *uint8, gcw *gcWork, shard int) {
	...
	// Scan this shard.扫描[b,b+n] 这个区间的所有对象 需要根据位图 ptrmask来判断
	// 将所有扫描到的对象加入gcw缓冲区等待标记
...
func gcDrain(gcw *gcWork, flags gcDrainFlags) {
	...
	// Drain root marking jobs. 这里大致意思是多个协程只有一个协程会去扫描全局root
	// 对于所有扫描到的根加入缓冲池中，在并发分配进行标记
	if work.markrootNext < work.markrootJobs {
		for !(preemptible && gp.preempt) {
			//挨着遍历扫描所有的段，数据段、bss段 栈，协程栈等
	...
	for !(preemptible && gp.preempt) {
		...
		//到这里都依然找到可用的灰色对象，所以可以结束当前流程了
		if b == 0 {
			// Unable to get work.
			break
		}
		//对灰色对象进行标记，递归扫描，如果做完了就标记为黑色对象
		scanobject(b, gcw)
...
func scanblock(b0, n0 uintptr, ptrmask *uint8, gcw *gcWork, stk *stackScanState) {
	...
	b := b0
	n := n0
	//开始扫描b0-n0区间的所有弟子，判断是否是堆里的对象，然后进行标记为灰色
	for i := uintptr(0); i < n; {
...
func scanobject(b uintptr, gcw *gcWork) {
	...
	//每次最大只可以扫描 128kb 内存 ,超过了就且分为多个该尺寸进行扫描
	//总之就是切分成多个块 然后重新加入灰色对象工作缓冲区等待 扫描标记为黑色
	if n > maxObletBytes {
		...
		if b == s.base() {
			...
			// uninitialized. 如果当前内存不需要扫描直接标记为黑色，之前已经移除了灰色队列，所以就不管了
	...
	var i uintptr
	//每个指针占8字节，所以在扫描内存的时候也是8字节进行间隔 扫描
	//这个n就是sizeclass，一般由68中type 1 - 32k大小的
