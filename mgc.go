...
const (
	_GCoff             = iota // GC not running; sweeping in background, write barrier disabled //gc没有运行 后台在清除，写屏障关闭
	_GCmark                   // GC marking roots and workbufs: allocate black, write barrier ENABLED //gc标记根和buff，
	_GCmarktermination        // GC mark termination: allocate black, P's help GC, write barrier ENABLED//gc标记结束，
)
...
//
// gcStart 将会从——GCoff状态转移为 _GCmark 开始标记状态
// 也有可能不会发生状态转移触发gc（在某些情况下，例如当前为系统栈上的调用,或者持有锁时）
func gcStart(trigger gcTrigger) {
	// 由于当一般在malloc，mallocgc中触发的调用，可能会已经获取了全局锁，所以不要尝试在不稳定的情况下调用
	// Since this is called from malloc and malloc is called in
	// the guts of a number of libraries that might be holding
	// locks, don't attempt to start GC in non-preemptible or
	// potentially unstable situations.
	mp := acquirem()
	//如果当前线程在其他地方也正在进行操作，则防止进入gc，且g0协程不能触发gc
	...
	// transition.
	//这里就是多线程争抢锁，拿到锁后发现状态已经不是_GC_OFF了则返回
	...
	// possible. 进入并发标记阶段 打开写屏障 开启新的一轮gc
	setGCPhase(_GCmark)
	...
	// block rather than returning to user code. 如果不是后台gc模式，则直接阻塞当前用户调用
	if mode != gcBackgroundMode {
		Gosched()
	}
...
func gcBgMarkStartWorkers() {
	...
	// each P has a background GC G. //确保每个p都有一个专用于标记的协程
	for _, p := range allp {
...
func gcBgMarkWorker(_p_ *p) {
	...
	for {
		// Go to sleep until woken by gcController.findRunnable. 大致是主动陷入休眠，等待gc开始时调度
		...
		// point, signal the main GC goroutine.如果所有的标记协程都陷入了等待且没有剩余工作，则标记阶段完成
		if incnwait == work.nproc && !gcMarkWorkAvailable(nil) {
			...
			releasem(park.m.ptr())
			//标记结束，每个标记协程都会检查自己是否是最后一个完成任务的协程
			gcMarkDone()
