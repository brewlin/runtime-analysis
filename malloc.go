...
const (
	...
	// memory.
	heapArenaBytes = 1 << logHeapArenaBytes // 每个area区域 一般由64MB大小的内存
...
func mallocgc(size uintptr, typ *_type, needzero bool) unsafe.Pointer { //主要的分配逻辑
	...
	if size <= maxSmallSize {                            //小于32k走默认内存分配
		if noscan && size < maxTinySize {
			...
			off := c.tinyoffset
			// Align tiny pointer for required (conservative) alignment. 对齐指针
			if size&7 == 0 {
				off = round(off, 8)
			} else if size&3 == 0 {
				off = round(off, 4)
...
func newobject(typ *_type) unsafe.Pointer {
	return mallocgc(typ.size, typ, true) //传入字节大小，类型等
}
